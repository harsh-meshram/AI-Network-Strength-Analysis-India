...
export const SignalMeasurementSchema = z.object({
    ...
    network_type: z.enum(['5G_SA', '5G_NSA', 'LTE', 'GSM', 'WCDMA', 'UNKNOWN']),
    h3_index: z.string().min(8).max(20),               // H3 res 11
    h3_index_res9: z.string().min(8).max(20),           // H3 res 9
    ...
    rsrp: z.number().int().min(-140).max(-44).nullable().optional(),
    rsrq: z.number().int().min(-20).max(-3).nullable().optional(),
    sinr: z.number().int().min(-23).max(40).nullable().optional(),
    ss_rsrp: z.number().int().min(-156).max(-31).nullable().optional(),
    ss_sinr: z.number().int().min(-23).max(40).nullable().optional(),
    dbm: z.number().int().min(-120).max(-20).nullable().optional(),
    asu_level: z.number().int().min(0).max(99).nullable().optional(),
    ...
    android_version: z.number().int().min(1).max(50).nullable().optional(),
